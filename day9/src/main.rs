use std::{cmp::Reverse, collections::{BTreeMap, BinaryHeap, HashMap}, path::PathBuf};
...
fn process1(input: &str) -> u64 {
    ...
    pos_length_files.into_iter().flat_map(|(pos, length, id)| (0..length).map(move |i| (pos+i)*id)).sum()
}
...
fn process2(input: &str) -> u64 {
    ...
    for id in (0..id).rev() {
        ...
        if let Some(pos_free) = pos_length_free.iter().take_while(|(pos_free,_)| pos_free< &&pos_file).find(|(_, length_free)| 
            length_free >= &&length_file
    ...
    pos_length_files.into_iter().flat_map(|(id, (pos,length))| (0..length).map(move |i| (pos+i)*id)).sum()
}
