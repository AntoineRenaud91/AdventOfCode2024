...
#[test]
fn test_parser() {
    assert_eq!(parse(TEST_INPUT),(vec![3,4,2,1,3,3],vec![4,3,5,3,9,3]));
}
...
fn process2(input:&str) -> i64 {
    ...
    let right = right.into_iter().fold(HashMap::<i64,i64>::new(),|mut map, r| {
        *map.entry(r).or_default()+=1;
        map
    });
    left.iter().filter_map(|l| right.get(l).map(|r| l*r)).sum()
}
...
fn main() {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).parent().unwrap().join("data/day1.dat");
