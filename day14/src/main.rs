...
fn parse(input: &str) -> impl Iterator<Item = ([i64;2], [i64;2])> + '_ {
    input
        ...
        .map(|line| {
            ...
            let p = p.trim_start_matches("p=").trim_end_matches('>').split(',').map(|x| x.parse().unwrap()).collect::<Vec<_>>();
...
fn process1(input: &str, nx: i64, ny: i64) -> usize {
    let quadrant = parse(input)
        .map(move |(pos,vel)| [(pos[0] + 100*vel[0]).rem_euclid(nx), (pos[1] + 100*vel[1]).rem_euclid(ny)])
...
fn process2(input: &str, nx: i64, ny: i64, nt: i64) -> Vec<Vec<char>> {
    parse(input)
        .map(move |(pos,vel)| [(pos[0] + nt*vel[0]).rem_euclid(nx), (pos[1] + nt*vel[1]).rem_euclid(ny)])
        .fold((0..ny).map(|_| (0..nx).map(|_| '.').collect::<Vec<char>>()).collect::<Vec<_>>(),|mut acc ,[i,j]| {
...
fn main() {
    ...
    for i in 0.. {
        ...
        let nlines_empty = message.iter().filter(|line| line.iter().all(|&c| c == '.')).count();
        let ncols_empty = (0..101).filter(|i| (0..103).all( |j| message[j][*i] == '.')).count();
