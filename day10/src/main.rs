...
fn parse(input: &str) -> Array2<u32> {
    ...
    Array2::from_shape_vec([n_rows, n_cols],
        input.trim().lines().flat_map(|line| line.trim().chars().map(|c| c.to_digit(10).unwrap())).collect()
    ).unwrap()
}
...
fn process_single1(map: &Array2<u32>, source: [usize; 2]) -> usize {
    ...
    paths.insert(source);
    for i in 1..10 {
        paths = paths.into_iter().flat_map(|ind| next_indices(ind, shape).into_iter().filter(|ind| map[*ind]==i)).collect();
...
fn process1(input: &str) -> usize {
    ...
    (0..n_rows).flat_map(|i| (0..n_cols).map(move |j| [i,j]))
...
fn process_single2(map: &Array2<u32>, source: [usize; 2]) -> usize {
    ...
    for i in 1..10 {
        paths = paths.into_iter().flat_map(|(ind,rating)| next_indices(ind, shape).into_iter().filter(|ind| map[*ind]==i).map(move|ind| (ind, rating)))
