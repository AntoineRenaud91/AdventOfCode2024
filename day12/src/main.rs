...
fn parse(input: &str) -> Array2<char>  {
    ...
    Array2::from_shape_vec([n_rows,n_cols], input.trim().lines().flat_map(|line| line.trim().chars()).collect()).unwrap()
}
...
impl Region {
    ...
    fn try_merge(&mut self, other: &Self) -> bool {
        ...
        self.edges = self.edges.symmetric_difference(&other.edges).copied().collect();
...
fn process1(input: &str) -> usize {
    ...
    regions.into_values().flat_map(|regions| regions.into_iter()).map(|r| r.area()*r.perimeter()).sum()
}
...
fn process2(input: &str) -> usize {
    ...
    regions.values().flat_map(|regions| regions.iter()).map(|r| r.area()*r.sides()).sum()
}
