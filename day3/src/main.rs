...
fn process1(input: &str) -> u64 {
    Regex::new(r"mul\(\d{1,3},\d{1,3}\)").unwrap()
        .find_iter(input)
        .map(|mat| {
            let (num1_str,num2_str) = mat.as_str().trim_start_matches("mul(").trim_end_matches(")").split_once(',').unwrap();
...
fn process2(input: &str) -> u64 {
    Regex::new(r"mul\(\d{1,3},\d{1,3}\)|don't\(\)|do\(\)").unwrap()
        ...
        .fold((0,true),|(sum,enabled), mat| {
            ...
            if mat_str == "do()" {(sum,true)}
            else if mat_str == "don't()" {(sum,false)}
            else if enabled {
                let (num1_str,num2_str) = mat_str.trim_start_matches("mul(").trim_end_matches(")").split_once(',').unwrap();
                (sum+num1_str.parse::<u64>().unwrap()*num2_str.parse::<u64>().unwrap(),true)
            }
            else {(sum,false)}
        }).0
}
...
fn main() {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).parent().unwrap().join("data/day3.dat");
